...
查出所有host 和hostid
select hostid,host  from  hosts where host not like '%Template%' and host not like '%CooHua%' and host not like '{%';

查出10204机器的 idle cpu的 itemid。
select itemid,name,key_ from items where hostid=10204 and key_ = 'system.cpu.util[,idle] ' ;

通过增额itemid 查
select min(value_min) from trends where itemid="27167" and from_unixtime(clock)>='2016-03-03' and from_unixtime(clock)<'2016-03-04' ;
...
select hosts.hostid,hosts.host,items.itemid,min(trends.value_min) from ( items left join hosts on hosts.host=items.host ) left join trends on items.itemid=trends.itemid where trends.from_unixtime(clock)>='2016-03-03' and trends.from_unixtime(clock)<'2016-03-04' ;
